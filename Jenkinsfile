      ...
      steps {
       container('kaniko') {
         script{
          ...
          sh "/kaniko/executor --insecure --skip-tls-verify --skip-tls-verify-pull --insecure-pull --dockerfile `pwd`/Dockerfile --context `pwd` --destination http://dev-registry.itmi.id/glm/itmi-core:${BUILD_NUMBER}"
          }
        }
      }
    }
